    ...
    while (done == False):
        ...
        if (right == True):
            if (pos == 1):
                ...
                contractL = contractMPSMPOL(
                    gs, hamil, gs, pos - 1, pos - 1, contractL[0])
        else:
            if (pos == L - 2):
                contractR = contractMPSMPOR(gs, hamil, gs, L - 1, L - 1, idMat)
                contractL = contractMPSMPOL(gs, hamil, gs, 0, L - 1, idMat)
            else:
                contractR = contractMPSMPOR(
                    gs, hamil, gs, pos + 1, pos + 1, contractR[0])
    print()
    return energy
...
def sumMPS(kets, coef, cutD, givenMPS=False, newMPS=0,
    ...
    while (done == False):
        ...
        if (right == True):
            ...
            SV = np.swapaxes(SV.reshape(
                (-1, newMPS.s[pos + 1], newMPS.sites[pos + 1].Dr)), 0, 1)
            newMPS.setA(pos, U.reshape(
                (newMPS.s[pos], newMPS.sites[pos].Dl, -1)))
            newMPS.setA(pos + 1, SV)
        else:
            US = np.einsum('ij,j->ij', U, S)
            Vdag = np.swapaxes(Vdag.reshape(
                (-1, newMPS.s[pos + 1], newMPS.sites[pos + 1].Dr)), 0, 1)
            newMPS.setA(pos, US.reshape(
                (newMPS.s[pos], newMPS.sites[pos].Dl, -1)))
        ...
        for i in range(num):
            if (right == True):
                if (pos == 1):
                    ...
                    contractR[i] = contractMPSR(
                        newMPS, kets[i], 0, L - 1, idMat)
                else:
                ...
                if (pos == L - 3):
                    contractR[i] = contractMPSR(
                        newMPS, kets[i], L - 1, L - 1, idMat)
                    contractL[i] = contractMPSL(
                        newMPS, kets[i], 0, L - 1, idMat)
                else:
